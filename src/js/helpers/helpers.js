...
ephHelpers.slugify = function(str) {
    var map = {
        '-' : ' ',
        '-' : '_',
        'a' : 'á|à|ã|â|À|Á|Ã|Â',
...
ephHelpers.formatCustomFields = function(customFields) {
    let customFieldsDef = []

    for (var i = 0; i < customFields.length; i++) {
      let cf = customFields[i]
      if (cf.type == "text" && cf.isVisible) {
        customFieldsDef.push({uuid:cf.prop, prop:cf.prop, displayAs:cf.name, edit:"true"})
      }
